...
| E8 | Lens — the unifying view surface (layout vs style axes; plugin-registerable) | needs-design | feature | M |  | PRODUCT CONCEPT (user, 2026-07-12): "a lens for your writing" — one first-class, summoned, non-destructive way of SEEING prose, unifying what are currently THREE efforts each inventing their own toggle/overlay surface: diagnostic lenses (harper/ltex-plus/vale — what's wrong), structural lenses (S6 ventilate view + rhythm gutter — the skeleton), stylistic lenses (S8 POS X-rays — what's flabby). THE DESIGN FORK: lenses split on TWO axes — STYLE lenses change how text is PAINTED and COMPOSE (adverbs dimmed AND spelling underlined is coherent); LAYOUT lenses change what is DRAWN and are EXCLUSIVE (two lenses cannot both re-break the same rows). Get that wrong and either diagnostics can't stack with POS, or two layout lenses fight. Effort-P plugins should be able to REGISTER lenses. Successor to E7 (Analysis view). URGENT: the in-flight multi-LSP work is its first implementer — its provider seam must NOT assume "provider == LSP subprocess" (S8's lens is in-process, synchronous, caret-local). |

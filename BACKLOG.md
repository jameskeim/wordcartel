...
| E14 | Engine-side suppression — persist disabled rules AND the personal dictionary, delivered through the engines' settings channels | triage | feature | M |  | ONE EFFORT (decided 2026-07-26) — absorbs the former E13 (ltex dictionary push), which is dropped as FOLDED-IN not rejected. Both halves push suppression state into an engine through the settings channel, need the same answer to WHERE IT PERSISTS, and need the same plumbing and test surface; filed apart the persistence argument gets had twice. RULE HALF (the part a writer is waiting for): the persistent form of E11 session dismiss — stop flagging a rule ever. User-visible and recurring; the most plausible next ask after E11. Probe-confirmed per engine: ltex disabledRules via the settings channel; harper the linters bool map it already pushes; vale has NO engine today (provider removed 2026-07-26, see E16) and would be a client-side code filter if the CLI provider lands. DICTIONARY HALF: adding a word ALREADY suppresses it across engines client-side before paint, so this buys suppression at the SOURCE only — zero user-visible change, against unbounded settings-payload growth, a second source of truth that must sync on REMOVAL too, and ltex per-language keying. Not worth doing alone; nearly free as a rider. GOVERNING FORK, shared: where does it persist? The app has never written config back; the only user-state files are dictionary.txt, settings-overrides.toml, session.toml — so a new user-state file or a settings-overrides extension, a call reaching beyond diagnostics. Answer it ONCE for both halves. Also owed: a command surface (contract law 2). Decision records: scratchpad/e11/decisions.md D6 + D7. |
| E16 | Vale via CLI one-shot on buffer content — replaces the vale-ls provider | triage | feature | M |  | THE VALE PATH, decided 2026-07-26 after a live probe reversed the 2026-07-25 arc choice (design-space OPEN 1). Supersedes E15 (dropped) and replaces the vale-ls provider, which is being REMOVED. Probe-established: vale-ls lints the file on DISK, never the synced buffer — a buffer-only error never flags, an on-disk error fixed in the buffer still reports; didChange yields ZERO server messages; didSave re-reads disk and IGNORES its own text param (proven by an out-of-band disk write); .vale.ini is directory-scoped (so a temp-file-elsewhere workaround applies the wrong rules). So no URI fix makes vale-ls follow typing. REPLACEMENT: the vale CLI one-shot over stdin — verified: piping to vale --no-exit --output=JSON --ext=.md returns Span/Check/Message/Severity/Match/Line/Link plus the Action object, everything a Diagnostic + Suggestion list needs, linting the text we hand it, giving vale the SAME live semantics as harper and ltex. Needs: the one-shot subprocess primitive (PD, whose hook already names vale-CLI as its proof case) or a narrower internal spawn — fork 1; a JSON-to-Diagnostic mapping; and running with cwd = the document directory or .vale.ini discovery finds the wrong config (the detail most likely to be missed). Drops the vale-ls dependency (user installs only vale). Reuses DiagSource::Vale, which stays reserved in core. Probe: scratchpad/e15/probe/vale-sync-probe-results.md. |
...
<details><summary>4 dropped</summary>
...
| E13 | Push the personal dictionary into ltex's settings channel (server-side spelling suppression) | FOLDED INTO E14, not rejected. Same effort: both push suppression state into an engine through the settings channel, both need the same answer to where it persists, both need the same delivery plumbing and test surface. Filed apart, the persistence argument gets had twice and the plumbing gets built twice. The dictionary half also does not stand alone — zero user-visible change against a real sync obligation — but is nearly free as a rider on the rule half. |
| E15 | vale is unreachable — doc_uri mints untitled: unconditionally and vale-ls publishes nothing for it | Superseded by E16. The live probe showed vale-ls lints the file on disk rather than the synced buffer, so no URI fix makes it a live lens; the vale-ls provider is being removed in favour of the vale CLI one-shot. |

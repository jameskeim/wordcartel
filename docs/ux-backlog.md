...
User report (2026-07-17): the menus and list modals are **wheel- and click-aware but not
hover-aware**. Moving the mouse over a dropdown/list item does nothing; the highlight only moves
via the wheel (one item at a time), and the highlighted item can only be chosen with RETURN. A
click already selects the item under the pointer correctly. The wheel-moves-highlight behavior is
*superfluous* once hover tracks selection — the wheel should just scroll the list.

**Settled model** (one coherent redesign of what the mouse means over an overlay; keyboard path
untouched):

| Input | Behavior |
|---|---|
| **Hover** (`Moved`) | highlight follows the pointer to the item under it, within the list rect |
| **Wheel** (`Scroll`) | scroll the **viewport**, then re-run the hover hit-test at the pointer position |
| **Click** (`Down`) | select the item under the pointer — **unchanged** (already correct) |
| **Arrows / Enter** | move highlight / select — **unchanged**, untouched by this effort |

Two load-bearing details:

- **A wheel scroll emits no `Moved` event** (the pointer is physically stationary; only content
  slides beneath it). So "the highlight follows the mouse" does not happen for free on a wheel
  scroll — the wheel handler must **re-run the hover hit-test itself** after adjusting `scroll_top`.
  With that, the highlight is pinned to whatever row sits under the pointer and can never scroll
  off-screen while the pointer stays over the list.
- **Reconciliation** (highlight now has two drivers, mouse *and* keyboard): when the pointer is
  **not** over the list, **leave the highlight where it is** — a keyboard-set highlight must stand.
  Hover simply doesn't act off-rect; whichever input last acted wins.

**Root cause of the current gap** (`mouse.rs`): every overlay mouse slot
(`mouse_menu`/`mouse_palette`/file-browser/diag/modals) handles only `ScrollDown`/`ScrollUp`
(→ move highlight) and `Down(Left)` (→ hit-test + dispatch). There is **no `MouseEventKind::Moved`
arm**, and `route_overlay` consumes and early-returns every mouse event while an overlay is open
(`mouse.rs:537`), so hover motions are silently dropped. The hit-testers already exist
(`chrome_geom::menu_bar_layout`, dropdown row geometry, `chrome_geom::palette_row_at`).

Continues the A1 / A6 / A13 mouse-interaction lineage. Adds no commands (command-surface contract:
N/A — does not touch the command surface). ~SM.

*(Captured 2026-07-17 via `scripts/backlog add`; re-anchored same day from "hover feedback" to the
full overlay mouse-interaction model during scoping.)*

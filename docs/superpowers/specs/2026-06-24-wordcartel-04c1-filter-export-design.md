...
- **Single mutation channel** (§10.1): a filter's document change happens **only**
  via `Buffer::apply` (a `ChangeSet` replacing the captured range) — the sanctioned
  way for an async result to change the document; it is **one undoable edit**.
...
- **Never crash / never lose work** (§15): the buffer is replaced **only** after a
  ...
  subprocess path.
- **Multi-buffer ready** (4r): a filter targets the active buffer and its result
  routes back by `buffer_id` (reuses 4r's `by_id` routing + version staleness).
- **Workspace facts:** `cargo test` from repo root; binary `wcartel`; new dep
  `subprocess`.
...
- **Typed filters** (minibuffer): the entered line is **argv-split on ASCII
  ...
  SelectionElseBuffer, .. }`. No shell, no pipes (pipelines need a `shell=true`
  preset, or Effort 5's richer UI).
...
1. **Invoke.** `filter` command opens the minibuffer (`> `). Enter builds a
   ...
   no minibuffer.
2. **Capture at dispatch (foreground).** `(buffer_id, version, input_range)` where
   `input_range` = the primary selection if non-empty else the whole buffer
   (`Filter`), the empty range at the cursor (`Insert`), or the whole buffer
   (`Export`). Read the stdin text (an O(1) rope snapshot materialized on the filter
   thread, not the foreground). Set status `running <cmd> ⏳`. Spawn the **dedicated
   filter thread**; store a shared cancel handle (`Arc<…>` + the `subprocess::Popen`
   behind a mutex, or a kill flag the thread polls) on the `Editor`. **One filter at
   a time** — a second invocation while one is in flight is rejected with a status.
3. **Run (filter thread).** `subprocess::Popen` with piped stdin/stdout;
   `communicate`/`communicate_start` feeds stdin and reads stdout deadlock-safely
   under `timeout`. Wrap: **Esc-cancel** (foreground sets the shared flag → the
   thread `terminate()`s/`kill()`s the child), **size-cap** (abort if stdout exceeds
   `max_output`), **UTF-8 validation** (reject binary). On completion the thread
   sends `Msg::FilterDone { buffer_id, version, range, outcome }` into the unified
   `Msg` channel and clears the in-flight handle.
4. **Merge (foreground, version-checked).** `reduce`'s `FilterDone` arm:
   - **Stale:** `by_id(buffer_id)` missing OR `document.version != version` →
     **discard**, status `filter discarded — buffer changed`. (First real consumer
     of 4b's version-discard staleness.)
   - **Success + fresh:**
     - `Filter` → `active_mut().apply(ChangeSet replacing range with output, …)` —
       one undoable edit; selection collapses to the replaced range's end.
     - `Insert` → `apply` inserting output at the cursor.
...
- `Editor.minibuffer: Option<Minibuffer>` — a **separate input mode** from the
  keypress modal `prompt` (which takes single-key choices). At most one of
  `prompt`/`minibuffer` is active; the minibuffer is for free text.
- `reduce` routes keys when `minibuffer.is_some()`: printable → insert at `cursor`;
  Backspace → delete before `cursor`; Left/Right → move `cursor`; Enter → submit
  (consume the text, clear the minibuffer, dispatch the filter); Esc → cancel
  (clear, no-op). Background `Msg::FilterDone`/`Tick`/`JobDone` still process under
  an active minibuffer (do not starve them — the §4b modal-interception lesson).
...
- **Output path is derived** from the active buffer's source path
  ...
  command reports "save the file first"). If the target exists, reuse the existing
  **confirm modal** before overwriting. Arbitrary output-path entry is **Effort 5**.
- `WritesOutput`: pass `-o <path>` as argv; we capture only stderr/exit.
  `Capture`: we read child stdout and atomically write `<path>` (reuse `file`/`swap`
  atomic-write discipline). Export never mutates the buffer or the source `.md`.

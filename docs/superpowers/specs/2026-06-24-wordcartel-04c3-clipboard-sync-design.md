...
- **New dependency (exact stanza — Codex CRITICAL/MINOR):**
  ...
  ```
  in `wordcartel/Cargo.toml` (shell crate only; NOT `wordcartel-core`). `default-features = false` drops `image-data` (we are text-only); `wayland-data-control` is REQUIRED for native-Wayland clipboard — without it arboard 3.x is X11-only and a Wayland-without-Xwayland session degrades to `NullBackend`. (Confirm the exact arboard 3.x version + that the feature name is current at implementation time.)

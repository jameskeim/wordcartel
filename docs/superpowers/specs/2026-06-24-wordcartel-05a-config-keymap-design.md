...
```
Unknown keys/sections are ignored with a **single** startup warning (forward-compat for later sub-efforts' keys). A TOML parse error → startup status warning naming the file + line, then fall back to defaults (never crash).
...
- **No timeout** (terminal key timing is unreliable). A pending sequence is resolved by the next key or cancelled by **Esc**. Esc with no pending sequence keeps its existing meaning (dismiss modal, etc., per the existing reduce order).
...
- `[keymap]` table: `"<chord-sequence>" = "<command-id>"`, chord-sequence is space-separated chords; chord grammar: `ctrl-`, `alt-`, `shift-` modifier prefixes + a key name (`a`..`z`, `f1`..`f12`, `enter`, `tab`, `esc`, `space`, `left`/`right`/`up`/`down`, `backspace`, `\\` etc.). Examples: `"ctrl-x" = "cut"`, `"ctrl-k ctrl-s" = "save"`.
- `"<chord>" = false` **unbinds** that chord/sequence.
- **Effective keymap** = the selected **preset base** (`keymap.preset`) → patched by the XDG `[keymap]` → patched by project-local `[keymap]` → patched by `--config`'s `[keymap]`. Each patch overrides/adds/unbinds individual sequences; it does NOT replace the whole map.
...
- **Validation:** every command-id string in the effective keymap is checked against the registry's known command ids at startup; an unknown id → a startup warning naming the chord+id, and that binding is dropped (never crash). A malformed chord string → same treatment.
...
`registry::CommandId(pub &'static str)` keys the registry, but config supplies **runtime** command-id strings. So the registry gains a name-resolution entry point — either `dispatch_by_name(&self, name: &str, ctx) -> Option<CommandResult>` or `resolve_name(&self, name: &str) -> Option<CommandId>` (impl choice for the plan; e.g. a `HashMap<&'static str, Handler>` view or `CommandId: Borrow<str>`). The keymap stores each binding's command-id as an owned string (from config) or a resolved `CommandId`; resolution + the §4.3 startup validation both go through this entry point, so the keymap never holds an id the registry doesn't know.

### 5.2 Replacing the hardcoded path
`input::key_to_command_id` (the hardcoded match → `KeyAction`) is replaced by keymap resolution: a `KeyEvent` becomes a `KeyChord`, appended to `pending_keys`, resolved against the active mode's trie; a `Resolution::Command(id)` dispatches via the registry (the existing `reduce` → `reg.dispatch` boundary). The printable-fallthrough (literal insert) is preserved for an unmodified single key that resolves to `None`.

## 6. Session state (resume + marks store)

- Stored in the XDG state dir (reuse `swap::state_dir()`, 0700), as a single state file (a map of **canonical absolute path** → `{ cursor: usize, scroll: usize, marks: Map<char,usize> }`). Single file (not per-document) keeps it simple and prunable.
- **Resume-at-position:** on opening a file with a stored entry and `state.resume = true`, restore cursor + scroll (clamped to the current document length — the file may have changed on disk). Out-of-range → clamp, don't error.
...
- **Config precedence:** built-in < XDG < project < `--config` (each overrides only its keys); `--no-config` → defaults only; malformed TOML → defaults + warning; unknown keys ignored with one warning. (Inject config dir/paths so tests don't touch real `$HOME`.)
- **Keymap:** chord parsing (modifiers, key names, sequences, bad strings); multi-key resolution (`Pending` → `Command`, Esc-cancel clears pending, unknown continuation → `None` + clear); patch override/add/unbind; preset selection (`cua` vs `wordstar` produce different effective maps); unknown command-id dropped + warned; printable fallthrough preserved.
- **Registry name resolution:** a known id resolves/dispatches; an unknown id does not.
- **Session state:** save→load round-trip restores cursor/scroll; resume clamps a past-EOF position; prune evicts beyond `max_entries`; scratch buffers not persisted; corrupt state file → empty store + warning; atomic write leaves no temp litter.

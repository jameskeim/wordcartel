...
Status: technical design revision 5 passed independent Codex review on 2026-09-14:
GO, 0 Critical / 0 Important / 0 Minor findings. D1–D6 remain user-approved; earlier
reports are preserved. Implementation planning may proceed.
[Final design review](../../reviews/2026-09-14-recovery-spec-review-round5.md).
...
## Technical proposal for review — revision 5
...
For a slot's first acknowledged checkpoint, strictly sync the full resolved chain
from owner directory through recovery-v2, state root, and ancestors to filesystem
root, whether those paths already exist or were created in this attempt. Retain this
first-checkpoint obligation across every failure; clear it only after full success.
Subsequent checkpoints may sync only owner directory. A new slot never assumes that
an existing directory created by another failed attempt is already durable. This
conservative rule also covers failure before slot publication and cross-slot retries.
...
BACKUP_SEMANTICS for directory handles, and reject REPARSE_POINT/nonregular metadata. Use the safe rustix::process::geteuid().as_raw() API (explicit Unix
rustix = "=1.1.4", process feature, cached/locked) for effective-UID validation; do not
introduce unsafe libc calls. A validated
...
Jobs use the existing FIFO executor with ResultClass::Durability. Extend Executor
with REQUIRED try_dispatch(Job)->Result<(), DispatchError>; preserve dispatch(Job)
as a default fire-and-forget wrapper for existing callers. All executor implementors
must implement try_dispatch; no optimistic default may claim acceptance. Inline runs
and buffers an outcome; Thread maps absent sender/send failure to rejection, dropping
the unsent Job and its captured guards. Recovery uses try_dispatch and handles Err
as an immediate terminal request failure, not an outstanding wait. Store the request
before dispatch (Inline may execute immediately), then on rejection remove exactly
that request/latch, preserve source, show failure, and progress/cancel batch according
to the same terminal table. Existing callers retain their historical dispatch semantics;
this is acceptance reporting, not a FIFO, scheduling, or shutdown redesign. A recovery merge
...
successful save snapshot and whose association is the committed destination. For Save As,
user approval on 2026-09-15 also permits a pathless association or the captured pre-rekey
association of this same editing instance. This exception does not relax ownership,
generation/version/body coverage or the strict same-handle destination receipt. Newer
...
failed user save. Target fingerprint must still match the committed
...
- Final revision-4 independent design gate: GO with zero findings; implementation
  planning authorized to proceed. Earlier pending-review notes describe their rounds.

- Revision 5 (plan-review clarifications): durable first-checkpoint ancestry sync
  obligations survive failed/reallocated slots; recovery uses explicit queue-rejection
  reporting; safe effective-UID API and target dependency are specified. Pending
  independent review with implementation plan revision 2.

- Final revision-5 independent design gate: GO, zero findings.

## Implementation clarification — contextual provenance

Task 3 uses current association for contextual discovery. Only when association is
absent does original recovery provenance provide the lookup context. Thus a recovered
unnamed successor remains discoverable when its original document is opened, while
a document saved under B is not also matched to historical A merely by provenance.
This does not assign a write target, grant cleanup rights, or automatically open a
recovery: selection and separate-document rules remain unchanged. Inactive owner
directories with no payload (only the permanent lock inode) are omitted; incomplete
temporary payloads and unreadable entries remain unavailable and preserved.


### Final review decisions — 2026-09-15

The user approved retiring an owned checkpoint after an accepted Save As strictly proves
its exact bytes durably saved at the new destination. The policy accepts only the committed
association, no association, or this editing instance's captured previous association.
Normal Save still requires the committed association. First recovered Save As and ordinary
A-to-B Save As can therefore finish cleanly without a repeated recovery offer. Rule-based
retention of a newer/divergent record is informational; IO, identity corruption and durability
uncertainty remain warnings. This supersedes the deliberately conservative clean-Save-As
retention interpretation in the earlier contract check.

The five-second timeout applies only while a foreground quit waits on concrete handoff or
association work (including a checkpoint blocking a queued association). Ordinary background
checkpoints and imports continue without timeout-driven cancellation. Detachment drops the
exit blocker, permits a matching late successful Ack to latch, and prevents late errors from
cancelling a subsequent quit; pending handoff retirement CAS is cancelled by the quit timeout.

The user also approved a bounded failure retry delay on 2026-09-15: failed automatic
checkpoints wait at least 30 seconds from the foreground completion clock before retrying.
A dedicated retry state supplies this floor independently of idle/max scheduling; edits
do not bypass it and successful protection resets it. This is the narrow R3 mitigation
approved in this effort; unrelated checkpoint scheduling remains deferred. Protocol
permission validation remains strict, with no automatic permission repair.

...
**Keybinding.** `Ctrl+A` in the `cua` preset (verified free in `CUA`). Universal
Select-All convention.

**Notes.** Pure selection-state change; no buffer edit, no derive/relayout needed
beyond `ensure_visible`. Reuses `wordcartel_core::selection::Selection::range`.
...
3. Valid → **clamp** to `[1, total_lines]` (line 0 or beyond-EOF never errors — it goes to the first/last line).
4. Move the caret to **column 1** (the start) of the target line:
   `caret = buffer.line_to_byte(line_index)` where `line_index = clamped - 1`.
5. Collapse the selection to the caret, then `nav::ensure_visible`.
...
**Implementation.** Pure render-side, computed from the caret each frame; no editor
state. Cost is bounded to the caret's logical line (one grapheme count over that line),
negligible on the per-frame path. Extend `render.rs`'s right-segment assembly (the
`word_count_segment` flush-right logic) to prepend the position; add a small helper
...
| `wordcartel/src/commands.rs` | `select_all` handler (whole-buffer selection + `ensure_visible`); `goto_line` action (parse/clamp/move) |
| `wordcartel/src/minibuffer.rs` | add `MinibufferKind` + `kind` field |
| `wordcartel/src/editor.rs` | `open_minibuffer(prompt, kind)` signature; init `kind` |
| `wordcartel/src/app.rs` | minibuffer submit routes on `kind` (Filter vs GotoLine) |
| `wordcartel/src/render.rs` | right-segment shows `Ln, Col` ahead of the word-count text; `caret_line_col` helper |

## 5. Testing

- **select_all:** whole non-empty buffer (range == 0..len, caret at end); empty buffer (empty selection, no panic).
- **goto_line:** valid mid-document line lands at that line's start; clamp-low (`0`/`1` → line 1); clamp-high (> total → last line); non-numeric → no-op + status; empty → no-op. Caret lands at column 1 (`line_to_byte`).

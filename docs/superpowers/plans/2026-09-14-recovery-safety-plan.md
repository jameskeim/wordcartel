...
Status: revision 2 passed independent Codex plan review on 2026-09-14: GO,
0 Critical / 0 Important / 0 Minor findings. P3 format discrepancy is governed by
the linked scoped format decision.
[Final plan gate](../../reviews/2026-09-14-recovery-plan-review-round2.md).
...
- Final independent plan gate (revision 2): GO, zero findings. Earlier pending-review
  statements describe their historical rounds; production implementation remains future work.


## Final review amendments — user approvals 2026-09-15

I2: The user approved strict receipt retirement on accepted Save As at a new destination.
Capture the editing instance's pre-rekey path; permit only the committed association,
that captured path, or no association. Preserve owner, generation, version, exact body,
same-handle fingerprint/file sync and destination-parent sync checks. First recovered
Save As and ordinary clean A-to-B Save As should retire covered checkpoints without a
warning; expected retention of newer/divergent records is Info, while IO and uncertainty
remain Warning. This supersedes the earlier clean-Save-As-retention test expectation.

I4: The user approved a bounded automatic retry delay in this effort. Failed checkpoints
arm a dedicated 30-second retry floor from the actual foreground completion clock; edits
do not bypass the floor, and success resets it. Do not automatically repair protocol
permissions. Other R3 scheduling changes remain deferred.

Final-review implementation corrections also scope the five-second timeout to foreground
quit waits, preserving matching late Acks; make unavailable rows dismissable per session
with manual review always available; retain unrelated contextual scans when closing a
picker; and borrow bodies during scans instead of materializing unused copies.

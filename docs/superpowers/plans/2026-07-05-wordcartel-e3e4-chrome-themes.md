...
**Architecture:** seven tasks along the spec's four seams — T1 core derivation + the two new faces; T2 the 19-at-once lineup (Codex r1 C1: no interim count ever breaks a gate) + its dependent shell-test rewrites; T3 resolve + the axis + Ansi16 policy + launch default; T4 the render split (mechanical); T5 the rewiring + reported-bug pins; T6 the toggle + persistence; T7 e2e + ship polish. Each task gates green independently.
...
### Task 2: the lineup, 19-at-once — phosphor/from_base16 edits, renames, terminal-ansi, the ten E4 themes, and every dependent shell-test rewrite

*(Codex plan r1 C1: an interim 9-name lineup leaves shell tests red for the rest of the
branch — the picker's >= 13, the a11y test's phosphor-amber-flat unwrap (render.rs:2129),
the phosphor border-RGB pins (render.rs:2623-2647), the resolve "default" name pins
(theme_resolve.rs:203/:206). The whole lineup therefore lands in ONE gate, WITH those
rewrites.)*
...
- Consumes: T1. Produces: phosphor constructors with NO chrome initializers (I4-A);
  `from_base16` with chrome/chrome_selected/chrome_muted as sentinels (chrome_reverse
  KEPT); `default()` named "terminal-plain" (table untouched — grounding E.7);
  `pub fn terminal_ansi()`; the TEN E4 constructors from grounding §C (byte-for-byte,
  source URLs in comments, chrome all-sentinel); `builtin()`/`builtin_names()` = the
  FULL 19 in D5's order; `-flat` gone.

**MID-SEQUENCE NOTE (honest intra-branch state):** until T3 wires derivation into
resolve, LIVE phosphor/E4 chrome renders unstyled (sentinel faces compose empty). Tests
that pin such chrome are rewritten IN THIS TASK to construct + `derive_chrome(Full)`
explicitly; the app's mid-branch visuals are imperfect but every gate is green. T3
completes the wiring.

- [ ] **Step 1: failing pins.** `phosphor_chrome_derives_fully` (phosphor-green +
  derive(Full) → grounding §B.3's phosphor hex — the ratified new look);
  `from_base16_chrome_derives` (probe palette → derived; chrome_reverse still
  reverse-modifier); `terminal_plain_name_and_faces` (name == "terminal-plain", faces
  byte-identical to today's table); `terminal_ansi_all_named_colors` (every face
  fg/bg named-ANSI or None; NOT monochrome; base Default); `builtin_names_final_nineteen`
  (D5's order — REWRITES all_thirteen_builtins_total); `every_builtin_resolves_at_all_depths`
  (19 × three depths through quantize, no panic); `derived_rungs_distinct_at_256`
  (grounding §B.5's expected indices); `all_color_themes_fully_explicit_after_derive`
  (Codex r1 I3 — the spec's completeness sweep: every Rgb-based builtin + derive(Full) →
  NO all-None color chrome face remains; terminal-*/no-color excluded by the non-Rgb
  rule); two exemplar spot pins (catppuccin-mocha base bg/fg + heading-1 == §C; flexoki-light
  ditto — the reviewer spot-checks two more at random per the [verify] discipline).
  TOKYO PIN (grounding E.2, resolved): tokyo sets ALL FOUR existing chrome faces —
  they all byte-survive derivation (incl. the hand-tuned DARK3 muted); only the two NEW
  faces derive. RED: constructors unchanged.
...
### Task 3: resolve + the axis + Ansi16 policy + launch default
...
**Interfaces:**
- Consumes: T1-T2. Produces: `resolve_theme(tc, env, disp: ChromeDisposition)` (EVERY call site + test gains the param — grounding A.2/E.1 enumerates them); `ThemeConfig.chrome: Option<String>` + RawTheme `chrome: Option<String>` — parsed IN RESOLVE via a small helper returning the disposition + an optional warning, the depth-key model exactly (Codex plan r1 I2; config stores strings, resolve interprets); the resolve order = base pick/construct → derive_chrome(disp) → THE ANSI16 POLICY (at Depth::Ansi16 on an Rgb-based theme, overwrite the five color faces with grounding §C's Ansi16 fixed table, binary-keyed) → user styles → cue glyph — user chrome overrides land LAST, over the depth policy (Codex plan r1 I1; Fable's spec r3 note); the "default"→terminal-plain alias warning; the "-flat"→base-phosphor fallback warning; the no-config arm returns `Theme::builtin("flexoki-dark")` (Depth::None still wins first); `Editor.chrome_disposition: ChromeDisposition` seeded in run().
...
### Task 4: the render split (mechanical)
...
### Task 5: the render rewiring + reported-bug pins

**Files:**
- Modify: `wordcartel/src/render_overlays.rs`, `wordcartel/src/render.rs` (status block + tests).

**Interfaces:**
- Consumes: T4's structure. Produces: every chrome surface on the six-face model per spec D2.
...
**Interfaces:**
- Consumes: T3's resolve signature + Editor field. Produces: `toggle_chrome` ("Chrome: Full/Zen", Settings menu, registered BEFORE save_settings — the journey rule); `editor.theme_rederive: bool`; the between-reduces rederive arm placed BEFORE the settings-save arm (Codex plan r1 C2: a same-cycle toggle+save must persist post-rederive state; the arm order in the loop region is rebuild → rederive → save); OTheme/MaskTheme.chrome + parse_mask passthrough + the plain diff_key theme-chrome arm + SettingsSnapshot.chrome_disposition + snapshots; the honest status arms.

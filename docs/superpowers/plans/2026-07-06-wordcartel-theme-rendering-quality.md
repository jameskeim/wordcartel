...
3. **Tokyo's T1 overlay pin is an intermediate value.** At T1, tokyo's `chrome`/`chrome_muted`/
   `chrome_selected` are still EXPLICIT (Part D lands in T3), and `chrome_muted.bg == None`, so at T1
   only `chrome_overlay` + `chrome_accent` derive, and the overlay elevates from the explicit
   `chrome.bg = PANEL_BG #16161e` (the muted-bg fallback). That intermediate overlay value is NOT in
   §II.5 (§II.5's tokyo pins are the T3 all-sentinel values); T1 pins it `[verify]` from the shipped
   code, and T3 REPLACES it with §II.5's `#4e5071`. Tokyo's accent (`#16161e/#8fa3ce`) is unchanged at
   T1 (its bg = explicit chrome.bg, its seed = tokyo BLUE link, both untouched until T3).
...
the four E3 semantic tests whose direction invariants are superseded.
...
- Consumers unchanged: the render `ChromeStyles` mapping, `face()`/`face_mut()`, the sentinel rule.
...
- [ ] **Step 1 (RED): the new pin battery + rewritten invariant tests.** Add/replace tests in the
  ...
  **1e. Re-hex the remaining derivation tests that keep their shape** (§I.9): `derive_preserves_hue_angle`
  ...
  Run: `cargo test -p wordcartel-core -- derive_ ` → RED (the tests reference the new invariants /
  pins the old code does not produce, and the base16 pins fail against the old split ladder).
...
- [ ] **Step 3 (GREEN + gates + commit).** Full `cargo test -p wordcartel-core -p wordcartel`
  (the render.rs tests that call `derive_chrome` for base16/phosphor still pass — their base16 pins
  are stable; the tokyo/phosphor render pins update in T3/T4). `cargo clippy --workspace --all-targets`
  clean; `cargo build` warning-free. Commit:
...
- [ ] **Step 1 (RED): tokyo face pins + the derived-chrome parity pins.** Add a `theme.rs` test pinning
  ...
  comment).
  Run `cargo test -p wordcartel-core -p wordcartel -- tokyo derive_fills` → RED.
...
- [ ] **Step 3 (gates + commit).** Full `cargo test -p wordcartel-core -p wordcartel`; clippy clean;
  build warning-free. Commit:

...
```
(`text` is the region slice already bound in `parse_region`; `range` is region-local, so index `text[range]` not the base-shifted `span`.) (c) `kind_to_role` adds `BlockKind::HtmlComment => Some(BlockRole::Comment),`. (d) Confirm `BlockRole::Comment` exists in `style.rs` (it must — add it if missing: `pub enum BlockRole { …, Comment }` and fix any exhaustive `BlockRole` match — grep).
...
```
(c) In `full_parse_src`/`parse_region` when `base == 0` (the whole-document parse): BEFORE the pulldown loop, if `front_matter_span(text)` is `Some(fm)`, push a `Block{ kind: FrontMatter, span: fm.clone(), children: vec![] }` and parse the REMAINDER `text[fm.end..]` with pulldown (shifting spans by `fm.end`). Implement as: detect fm; if present, emit the FrontMatter block, then run the existing event loop over `&text[fm.end..]` with a base offset of `fm.end`. (Keep the non-front-matter path unchanged.) `front_matter_span` returns None for a mid-doc `---` because `strip_prefix("---\n")` requires byte 0. (d) `kind_to_role`: `BlockKind::FrontMatter => Some(BlockRole::FrontMatter),`.
...
```
> The implementer: wire this beside the existing `html_in_play` check in the incremental update path so a true result takes the same full-reparse branch (`WidenReason::WidenToEnd` / full reparse). The exact `Edit` field names (`edit.range`, `edit.delta()`) and the `TextSource::slice` signature are in the surrounding code — match them. The CONTRACT is the oracle below; get the trigger broad enough that every oracle case passes (when in doubt, reparse from 0 — front matter is a tiny region at the doc head, so the cost is negligible).
...
**Files:** `wordcartel-core/src/md_parse.rs` (`apply_block_prefix_conceal`); test.

**Interfaces:** `apply_block_prefix_conceal` returns `Some("▎ ")` for BlockQuote and `Some("─── ")` (or a full-width rule) for ThematicBreak (previously both `None`).
...
- [ ] **Step 3: Implement.** In `apply_block_prefix_conceal`, the `BlockRole::BlockQuote` arm: after concealing `>` + space, `return Some("▎ ".to_string());` (was `None`). The `BlockRole::ThematicBreak` arm: after concealing the line, `return Some("─── ".to_string());` (was `None`).

- [ ] **Step 4: Run** `cargo test -p wordcartel-core md_parse:: blockquote thematic` — PASS. Then `cargo test -p wordcartel-core` — fix any EXISTING md_parse test that asserted blockquote/thematic prefix_glyph == None (those goldens are intentionally updated: the glyphs are new). Update such a test to expect the glyph.

- [ ] **Step 5: Commit** `feat(theme): blockquote ▎ + thematic-break ─── prefix glyphs`
...
```rust
    ...
    let prefix_width: usize = prefix_glyph.as_deref()
        .map(|g| g.chars().map(grapheme_width_char).sum()) // use the same width fn as cells
        .unwrap_or(0);
```
> Use the SAME display-width function the layout already uses for cells (`grapheme_width`/`unicode_width`) so `prefix_width` is the true painted width of the glyph string. The glyphs are `"• "` / `"▎ "` / `"─── "` (widths 2/2/4) and the heading glyph (Task 7).
...
> NO render change is needed for the cursor: render still paints the prefix glyph span then the text spans sequentially (so the painted output is identical except wrapped-continuation indent), and `nav::screen_pos` reads `source_to_visual` which now returns prefix-inclusive `col` → `frame.set_cursor_position(text_left + col)` lands on the text. Mouse `offset_at_cell` passes the visual `col` to `visual_to_source` which now clamps the prefix region.
...
**Files:** `wordcartel-core/src/layout.rs` (the `heading_prefix` input); `wordcartel/src/derive.rs` + `wordcartel/src/nav.rs` (`layout(...)` call sites); `wordcartel/src/render.rs` (fill the glyph); test.
...
**Files:** `wordcartel/src/render.rs` (placed-path + segs-path glyph loops); `editor.rs`/test.
...
- [ ] **Step 2: Run** `cargo test -p wordcartel a11y_` — these assert CURRENT behavior after Tasks 1-9; they should PASS. If `a11y_structural_glyphs_render_in_no_color` fails, a glyph isn't reaching the buffer — that's a real §13.2 gap to fix in the relevant producer/render task, not a test to weaken.

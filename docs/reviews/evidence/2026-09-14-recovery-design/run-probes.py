...
assert b'mod recovery_design_probes {' not in original, 'probes already installed; inspect before running'
log = evidence / (sys.argv[1] if len(sys.argv) > 1 else 'probes.log')
cmd = ['cargo', 'test', '-p', 'wordcartel', '--lib', 'recovery_design_probes', '--', '--test-threads=1']

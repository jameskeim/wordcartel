...
Status: complete technical proposal, revision 2, awaiting independent re-review.
...
## Technical proposal for review — revision 2
...
read/compare its complete capped bytes to the captured saved snapshot, sync that file
handle, then strictly sync its parent. Do this for Saved AND Unchanged. Failure or
...
A shared atomic request state has Pending, Cancelled, Retiring, Finished. After
...
Checkpoint failure/panic terminates this attempt, releases the source lease, keeps
the original, marks the open document's protection failure visibly, and advances to
...
| Combined checkpoint failure/panic | Release source on job unwind/end; remove request on outcome; mark protection failed; advance next | Cancel an active waiting quit with visible failure; later Quit may proceed through normal dirty review |
...
| Five-second pending-work timeout | CAS Pending->Cancelled; detach request from exit blockers, retain it for eventual safe completion routing; show timeout | Cancel current quit once; subsequent quit may proceed; worker can only retire if CAS already won with durable successor |

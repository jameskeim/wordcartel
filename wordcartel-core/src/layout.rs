...
mod tests {
    ...
    fn word_wrap_breaks_at_space_not_midword() {
        ...
        let (rows, _) = layout("hello wide", BlockRole::Paragraph, LineRender::Concealed, 8, false, 0);
    ...
    fn word_wrap_fallback_when_no_opportunity() {
        ...
        let (rows, _) = layout("abcdef", BlockRole::Paragraph, LineRender::Concealed, 4, false, 0);
        assert_eq!(rows.len(), 2);
    ...
    fn word_wrap_codeblock_keeps_grapheme_wrap() {
        ...
        let (rows, _) = layout("let x = 1;", BlockRole::CodeBlock, LineRender::Concealed, 4, false, 0);
    ...
    fn word_wrap_long_url_falls_back() {
        ...
        let (rows, _) = layout("https://example.com/aaaa", BlockRole::Paragraph, LineRender::Concealed, 8, false, 0);
        assert!(rows.len() >= 3);
    ...
    fn word_wrap_cjk_mixed_script() {
        ...
        let (rows, _) = layout("中文混排English", BlockRole::Paragraph, LineRender::Concealed, 6, false, 0);
    ...
    fn word_wrap_repeat_zero_width_head_no_overwide_row() {
        ...
        let (rows, _) = layout("\u{200b}ab", BlockRole::Paragraph, LineRender::Concealed, 1, false, 0);
    ...
    fn word_wrap_break_at_row_start_falls_back() {
        ...
        let (rows, _) = layout(" abcdefgh", BlockRole::Paragraph, LineRender::Concealed, 4, false, 0);
    ...
    fn active_line_identity_and_wrap() {
        ...
        let (rows, map) = layout("abcdef", BlockRole::Paragraph, LineRender::RawPlain, 4, false, 0);
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].display, "abcd");
        assert_eq!(rows[1].display, "ef");
        assert_eq!(map.eol, 6);
    ...
    fn concealed_bold_drops_markers_in_display() {
        ...
        let (rows, _map) = layout("**bold**", BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
    ...
    #[test]
    fn cjk_width_two() {
        let (rows, _) = layout("中a", BlockRole::Paragraph, LineRender::RawPlain, 80, false, 0);
    ...
    fn style_attached_to_placed() {
        ...
        let (_rows, map) = layout("**bold**", BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
    ...
    fn roundtrip_bijection_on_visible_cells() {
        let (_rows, map) = layout("a中b", BlockRole::Paragraph, LineRender::RawPlain, 80, false, 0);
        for p in &map.placed {
    ...
    fn cursor_never_inside_concealed_marker() {
        ...
        let (_rows, map) = layout("**a**", BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
        let stops = map.cursor_stops();
    ...
    fn end_of_row_clamps_not_teleports() {
        ...
        let (_rows, map) = layout("abcd", BlockRole::Paragraph, LineRender::RawPlain, 2, false, 0);
    ...
    fn right_skips_concealed_link_url() {
        ...
        let (_r, map) = layout(line, BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
    ...
    fn move_end_snaps_off_concealed_trailing_marker() {
        ...
        let (_r, map) = layout("**a**", BlockRole::Paragraph, LineRender::Concealed, 1, false, 0);
    ...
    fn styled_segments_split_by_style() {
        ...
        let (rows, _map) = layout("a **b**", BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
    ...
    fn move_left_from_leftmost_stays_on_visible_stop() {
        ...
        let (_r, map) = layout("**a**", BlockRole::Paragraph, LineRender::Concealed, 80, false, 0);
    ...
    fn rows_carry_block_role_and_glyph() {
        let (rows, _m) = layout("- item", BlockRole::ListItem, LineRender::Concealed, 80, false, 0);
    ...
    fn heading_rows_carry_heading_role() {
        let (rows, _m) = layout("## Title", BlockRole::Heading(2), LineRender::Concealed, 80, false, 0);
    ...
    fn prefix_offsets_columns_so_cursor_lands_on_text() {
        ...
        let (_rows, map) = layout("- item", BlockRole::ListItem, LineRender::Concealed, 40, false, 0);
        assert_eq!(map.prefix_width, 2, "• + space");
    ...
    fn no_prefix_is_unchanged() {
        let (_rows, map) = layout("plain text", BlockRole::Paragraph, LineRender::Concealed, 40, false, 0);
    ...
    fn prefix_reduces_wrap_capacity() {
        ...
        let (rows, map) = layout("- aaaa bbbb", BlockRole::ListItem, LineRender::Concealed, 6, false, 0);
    ...
    fn down_then_up_preserves_desired_col() {
        ...
        let (_r, map) = layout("aaaaa", BlockRole::Paragraph, LineRender::RawPlain, 3, false, 0);
    ...
    fn heading_prefix_reserves_width_when_on() {
        let (_r, on)  = layout("## Title", BlockRole::Heading(2), LineRender::Concealed, 40, true, 0);
        let (_r, off) = layout("## Title", BlockRole::Heading(2), LineRender::Concealed, 40, false, 0);
    ...
    fn heading_prefix_off_for_non_heading() {
        let (_r, m) = layout("para", BlockRole::Paragraph, LineRender::Concealed, 40, true, 0);
    ...
    fn law_w2_nested_prefix_alignment_inactive() {
        for (line, marker_w) in [("- x", 2), ("  - x", 2), ("    - x", 2), ("\t- x", 2),
            ...
            let (rows, map) = layout(line, BlockRole::ListItem, LineRender::Concealed, 20, false, 0);
    ...
    fn concealed_layout_style_unchanged_under_last_match() {
        ...
        let (rows, _) = layout("**bold**", BlockRole::Paragraph, LineRender::Concealed, 40, false, 0);
...
mod props {
    ...
    proptest! {
        ...
        fn law1_colmap_roundtrip(
            ...
            let (_rows, map) = layout(&line, BlockRole::Paragraph, render, w, false, 0);
            for p in &map.placed {
        ...
        fn law3_softwrap_fidelity(
            ...
            let (rows, map) = layout(&line, BlockRole::Paragraph, render, w, false, 0);
        ...
        fn law4_active_identity(line in logical_line(), w in widths()) {
            let (_rows, map) = layout(&line, BlockRole::Paragraph, LineRender::RawPlain, w, false, 0);
        ...
        fn law6_all_nav_ops_land_on_stop_concealed(
            ...
            let (_rows, map) = layout(&line, BlockRole::Paragraph, LineRender::Concealed, w, false, 0);
            let valid: std::collections::HashSet<usize> =
                map.placed.iter().map(|p| p.src.start)
                    .chain(std::iter::once(map.eol))
                    .collect();

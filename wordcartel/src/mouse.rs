...
pub(crate) fn mouse_menu(editor: &mut Editor, ev: MouseEvent, area: ratatui::layout::Rect,
    ...
    if let MouseEventKind::Down(MouseButton::Left) = ev.kind {
        ...
        if let Some(cat) = bar_hit {
            ...
            crate::menu::dispatch_row_action(editor, ctx, action);
        } else {
        ...
        if let MouseEventKind::Moved = ev.kind {
            if ev.row > 0 {
                ...
                if editor.mouse.menu_bar_revealed && editor.mouse.menu_hide_due.is_none() {
                    editor.mouse.menu_hide_due = Some(ctx.clock.now_ms() + MENU_LEAVE_GRACE_MS);
                }
            } else {
                ...
                {
                    editor.mouse.menu_reveal_due = Some(ctx.clock.now_ms() + MENU_DWELL_MS);
                }
            }
        }
    }
    ...
    if editor.scrollbar_mode == crate::config::TransientMode::Auto {
        if let MouseEventKind::Moved = ev.kind {
            ...
            if at_right_edge {
                ...
                {
                    editor.mouse.scrollbar_reveal_due = Some(ctx.clock.now_ms() + MENU_DWELL_MS);
                }
            } else {
                editor.mouse.scrollbar_reveal_due = None;
                if editor.mouse.scrollbar_revealed && editor.mouse.scrollbar_hide_due.is_none() {
                    editor.mouse.scrollbar_hide_due = Some(ctx.clock.now_ms() + MENU_LEAVE_GRACE_MS);
                }
            }
        }
    }
        ...
        if let MouseEventKind::Moved = ev.kind {
            ...
            if at_bottom {
                ...
                if !editor.mouse.status_revealed
                    ...
                    && no_overlay_open(editor)
                {
                    editor.mouse.status_reveal_due = Some(ctx.clock.now_ms() + MENU_DWELL_MS);
                }
            } else {
                editor.mouse.status_reveal_due = None;
                if editor.mouse.status_revealed && editor.mouse.status_hide_due.is_none() {
                    editor.mouse.status_hide_due = Some(ctx.clock.now_ms() + MENU_LEAVE_GRACE_MS);
                }
            }
        }
    }
    match ev.kind {
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            if let CellHit::MenuBar = hit {
                ...
                editor.mouse.scrollbar_until_ms = ctx.clock.now_ms() + 1200;
...
mod tests {
    ...
    // H27 test seam: `handle` collapsed onto `&DispatchCtx` in production; this helper builds
    // the ctx from the pre-H27 loose args so every existing call site needs only a name change
    // (handle → handle_flat), never an argument edit — the 12 sites with a per-call inline
    // clock/fs stay correct because those values still flow through THIS fn's own params.
    #[allow(clippy::too_many_arguments)]
    fn handle_flat(
        e: &mut Editor,
        ev: MouseEvent,
        reg: &crate::registry::Registry,
        km: &crate::keymap::KeyTrie,
        ex: &dyn crate::jobs::Executor,
        clock: &dyn wordcartel_core::history::Clock,
        tx: &std::sync::mpsc::Sender<crate::app::Msg>,
        fs: &std::sync::Arc<dyn crate::fsx::Fs + Send + Sync>,
    ) {
        let ctx = crate::overlays::DispatchCtx { reg, keymap: km, ex, clock, msg_tx: tx, fs };
        super::handle(e, ev, &ctx);
    }
    ...
    fn click_places_caret_at_cell_offset() {
        ...
        handle_flat(&mut e, down(1, 1), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn click_below_content_goes_to_doc_end() {
        ...
        crate::derive::rebuild(&mut e);
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, down(0, 10), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // row past content
    ...
    fn mouse_click_below_folded_tail_snaps_to_heading() {
        ...
        handle_flat(&mut e, down(0, 10), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn mouse_ignored_during_pending_mark() {
        ...
        crate::derive::rebuild(&mut e);
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, down(1, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn drag_selects_range_from_anchor() {
        ...
        crate::derive::rebuild(&mut e);
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, down(1, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // anchor at offset 1
        let drag = MouseEvent { kind: MouseEventKind::Drag(MouseButton::Left), column: 4, row: 0, modifiers: KeyModifiers::NONE };
        handle_flat(&mut e, drag, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // head at offset 4
    ...
    fn shift_click_extends_keeping_anchor() {
        ...
        handle_flat(&mut e, shift_down, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        let r = e.active().document.selection.primary();
    ...
    fn double_click_selects_word_triple_selects_paragraph() {
        ...
        handle_flat(&mut e, down(7, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        handle_flat(&mut e, down(7, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        let r = e.active().document.selection.primary();
        assert_eq!(e.active().document.buffer.slice(r.from()..r.to()), "beta");
        handle_flat(&mut e, down(7, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // triple → paragraph
    ...
    fn wheel_scrolls_view_not_caret() {
        ...
        handle_flat(&mut e, wheel, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn wheel_moves_selection_and_window() {
        ...
        for _ in 0..20 {
            handle_flat(&mut e, scroll_down, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        }
    ...
    fn click_outside_palette_closes_it() {
        ...
        e.palette = Some(crate::palette::Palette::default());
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, down(0, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // top-left, outside the centered overlay
    ...
    fn click_on_inactive_bar_opens_that_category() {
        ...
        handle_flat(&mut e, down(col, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn dwell_arms_on_row0_rest() {
        ...
        handle_flat(&mut e, moved(5, 0), &reg, &km, &ex, &TestClock(0), &tx, &crate::test_support::test_fs());
    ...
    fn dwell_rearm_tracks_last_motion() {
        ...
        e.menu_bar_mode = crate::config::MenuBarMode::Auto;
        handle_flat(&mut e, moved(5, 0), &reg, &km, &ex, &TestClock(0), &tx, &crate::test_support::test_fs());
        handle_flat(&mut e, moved(6, 0), &reg, &km, &ex, &TestClock(100), &tx, &crate::test_support::test_fs());
    ...
    fn leave_arms_grace_once() {
        ...
        handle_flat(&mut e, moved(5, 5), &reg, &km, &ex, &TestClock(0), &tx, &crate::test_support::test_fs());
        assert_eq!(e.mouse.menu_hide_due, Some(MENU_LEAVE_GRACE_MS),
            "first leave motion must arm the grace deadline");
        handle_flat(&mut e, moved(6, 5), &reg, &km, &ex, &TestClock(100), &tx, &crate::test_support::test_fs());
        assert_eq!(e.mouse.menu_hide_due, Some(MENU_LEAVE_GRACE_MS),
    ...
    fn reentry_cancels_grace() {
        ...
        handle_flat(&mut e, moved(5, 0), &reg, &km, &ex, &TestClock(100), &tx, &crate::test_support::test_fs());
    ...
    fn scrollbar_dwell_arms_on_right_edge_rest() {
        ...
        let (reg, ex, _, tx, km) = ctx();
        handle_flat(&mut e, moved(39, 4), &reg, &km, &ex, &TestClock(0), &tx, &crate::test_support::test_fs()); // col w-1 = 39
    ...
    fn wheel_never_arms() {
        ...
        handle_flat(&mut e, wheel_up, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn click_outside_cursor_picker_closes_and_restores() {
        ...
        let (reg, ex, clk, tx, km) = ctx();
        // (0, 0) is well outside the centered overlay — click-away.
        handle_flat(&mut e, down(0, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn cursor_picker_hover_previews_the_row() {
        ...
        handle_flat(&mut e, moved(r.x + 1, r.y + 1 + 3), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn cursor_picker_hover_same_row_does_not_re_preview() {
        ...
        let n = crate::cursor_picker::ROW_ACTIONS.len();
        let r = crate::chrome_geom::palette_overlay_rect(area, n + 1);
        handle_flat(&mut e, moved(r.x + 1, r.y + 1 + 3), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // preview row 3
        e.set_caret_shape(crate::config::CaretShape::Default); // tamper
        handle_flat(&mut e, moved(r.x + 1, r.y + 1 + 3), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // SAME row again
    ...
    fn cursor_picker_wheel_empty_guard_and_theme_restore() {
        ...
        for row in 1..=3u16 { handle_flat(&mut e, moved(r.x + 1, r.y + 1 + row), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); }
    ...
    fn theme_picker_wheel_empty_list_no_preview() {
        ...
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, wheel_ev(true, 40, 12), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        handle_flat(&mut e, wheel_ev(false, 40, 12), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn cursor_picker_wheel_boundary_notch_fires_no_preview() {
        ...
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, wheel_ev(true, 0, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // down at bottom → no move
    ...
    fn theme_picker_wheel_boundary_notch_fires_no_preview() {
        ...
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, wheel_ev(false, 0, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // up at 0 → no move
    ...
    fn cursor_picker_hover_fires_preview_exactly_once_per_row_crossed() {
        ...
        for row in [1u16, 3, 5] {
            ...
            handle_flat(&mut e, moved(r.x + 1, r.y + 1 + row), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn click_dir_enters() {
        ...
        let d = MouseEvent { kind: MouseEventKind::Down(MouseButton::Left), column: rect.x + 1, row: click_row, modifiers: KeyModifiers::NONE };
        handle_flat(&mut e, d, &reg, &km, &ex, &clk, &tx, &fs);
    ...
    fn click_on_an_available_recents_row_opens_it_like_enter() {
        ...
        handle_flat(&mut e, d_ev, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn menu_wheel_scrolls_dropdown() {
        ...
        for _ in 0..10 { handle_flat(&mut e, wheel, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); }
    ...
    fn menu_click_on_painted_indicator_row_does_not_dispatch() {
        ...
        handle_flat(&mut e, click, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn minibuffer_click_positions_caret() {
        ...
        let status_row = h - 1;
        handle_flat(&mut e, down(4 + 1, status_row), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn minibuffer_click_past_end_clamps() {
        ...
        let (_w, h) = e.active().view.area;
        let status_row = h - 1;
        handle_flat(&mut e, down(4 + 50, status_row), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // far past the text
    ...
    fn minibuffer_click_prompt_is_noop() {
        ...
        let (_w, h) = e.active().view.area;
        let status_row = h - 1;
        handle_flat(&mut e, down(1, status_row), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // col 1 < prompt_cols (4)
    ...
    fn search_match_click_refreshes_stale_cache() {
        ...
        handle_flat(&mut e, miss, &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn palette_hover_moves_highlight_to_pointer_row() {
        ...
        handle_flat(&mut e, moved(rect.x + 1, rect.y + 2 + 3), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn palette_hover_off_rect_leaves_highlight() {
        ...
        handle_flat(&mut e, moved(0, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); // top-left, off the overlay
    ...
    fn palette_wheel_scrolls_and_re_hovers() {
        ...
        handle_flat(&mut e, wheel_ev(true, rect.x + 1, rect.y + 2), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn outline_hover_does_not_jump() {
        ...
        let rect = crate::chrome_geom::palette_overlay_rect(area, n);
        handle_flat(&mut e, moved(rect.x + 1, rect.y + 2 + 1), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn diag_hover_does_not_apply() {
        ...
        handle_flat(&mut e, moved(rect.x + 1, rect.y + 1 + 1), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn menu_hover_bar_switches_category_with_reset_triple() {
        ...
        handle_flat(&mut e, moved(other_rect.x, other_rect.y), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        let m = e.menu.as_ref().unwrap();
    ...
    fn menu_hover_same_category_does_not_reset() {
        ...
        handle_flat(&mut e, moved(own_rect.x, own_rect.y), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        let m = e.menu.as_ref().unwrap();
    ...
    fn menu_hover_dropdown_row_sets_highlight() {
        ...
        handle_flat(&mut e, moved(drop.x, drop.y + 1), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn menu_hover_bar_with_no_menu_open_does_not_open() {
        ...
        handle_flat(&mut e, moved(2, 0), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
    ...
    fn menu_wheel_tall_category_scrolls_without_landing_on_indicator_row() {
        ...
        for _ in 0..3 { handle_flat(&mut e, wheel_ev(true, 0, 7), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs()); }
    ...
    fn menu_wheel_short_category_steps_by_one() {
        ...
        let (reg, ex, clk, tx, km) = ctx();
        handle_flat(&mut e, wheel_ev(true, 0, 5), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
        ...
        handle_flat(&mut e, wheel_ev(false, 0, 5), &reg, &km, &ex, &clk, &tx, &crate::test_support::test_fs());
